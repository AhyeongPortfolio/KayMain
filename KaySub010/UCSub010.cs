...
namespace KaySub010
{
    public partial class UserControl1: UserControl
    {
        ...
        public UserControl1()
        {
            ...
            ct_lic_empno.TextChanged += InputData_TextChanged;
        ...
        private void UserControl1_Load(object sender, EventArgs e)
        {
            //*----ComboBox 채우기---------------------------------------------
            //*--콤보박스 채우기-----------------------------------------
            Utility.SetComboWithCdnm(ct_lic_name, SQLStatement.SelectSQL2);
            //Utility.SetComboWithCdnm(ct_lic_dept, SQLStatement.SelectSQL3);
        ...
        public void BtnInsert_Click()
        {
            ...
            //---날짜 리셋----------------------------------------------
            //ct_lic_.Text = System.DateTime.Now.ToString("yyyyMMdd");

            //*--사원번호 리셋------------------------------------------
            var rowIdx2 = dataGridView1.CurrentRow == null ? 0 : dataGridView1.CurrentRow.Index;
            if (dataGridView1.Rows.Count == 0) ct_lic_empno.ReadOnly = false;
            else ct_lic_empno.ReadOnly = true;

            ct_lic_empno.Text = dataGridView1.Rows[rowIdx2].Cells["bas_empno"].Value.ToString();
        ...
        public void BtnSave_Click()
        {
            ...
            try
            {
                ...
                foreach (DataGridViewRow row in dataGridView2.Rows)
                {
                    ...
                    cmd.Parameters.Add("lic_NAME", OracleDbType.Varchar2).Value = Utility.GetCodeNM(row.Cells["lic_name"].Value.ToString());
                    cmd.Parameters.Add("lic_GRADE", OracleDbType.Varchar2).Value = row.Cells["lic_grade"].Value;
                    cmd.Parameters.Add("lic_ACQDATE", OracleDbType.Varchar2).Value = Utility.FormatMonthR(row.Cells["lic_acqdate"].Value.ToString());                    
            ...
            foreach (DataGridViewRow row in dataGridView2.Rows)
            {
                ...
                row.Cells["Key2"].Value = Utility.GetCodeNM(row.Cells["lic_name"].Value.ToString());
        ...
        private void DataGridView1_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    row.Cells["lic_acqdate"].Value = Utility.FormatMonth(dr["lic_acqdate"].ToString());
        ...
        private void Input_Validation_Check(object sender, EventArgs e)
        {
            ...
            //*---------------------------------------------------------------------------------------------------------
            if (string.IsNullOrEmpty(ct_lic_empno.Text))
            {
                SetError(ct_lic_empno, "입력하실 사원을 선택해주세요", dataGridView2.SelectedRows[0], errorProvider1);
            }
            else
            {
                ResetError(ct_lic_empno, errorProvider1);
            }

            //*---------------------------------------------------------------------------------------------------------
            if (string.IsNullOrEmpty(ct_lic_name.Text))
            {
                SetError(ct_lic_name, "자격증을 반드시 선택해주세요", dataGridView2.SelectedRows[0], errorProvider1);
            }
            else
            {
            ...
            //*---------------------------------------------------------------------------------------------------------
            if (string.IsNullOrEmpty(ct_lic_acqdate.Text))
            {
                SetError(ct_lic_acqdate, "취득일자를 반드시 입력해주세요", dataGridView2.SelectedRows[0], errorProvider1);
            }
            else
            {
