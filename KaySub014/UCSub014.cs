...
namespace KaySub014
{
    ...
    /// --Form Name           : 인사발령등록 프로그램 
    /// --최근작성 정보
    /// 1. 2021-08-31             권아영             폼생성
    /// 2. 2021-09-08             권아영             코드 수정 및 쿼리문 생성
    /// **********************************************************************
    /// </summary>
    /// 
    public partial class UserControl1: UserControl
    {
        ...
        private void UserControl1_Load(object sender, EventArgs e)
        {
            ...
            //*--콤보박스 채우기-----------------------------------------
            Utility.SetComboWithCdnm(ct_papp_dept, SQLStatement.SelectSQL3);
            Utility.SetComboWithCdnm(ct_papp_pos, SQLStatement.SelectSQL4);
        ...
        public void BtnSearch_Click()
        {
            ...
            try
            {
                ...
                cmd.BindByName = true;
                cmd.Parameters.Add("papp_appno", OracleDbType.Varchar2).Value = "%" + noSearch.Text + "%";
                cmd.Parameters.Add("papp_date1", OracleDbType.Varchar2).Value = Utility.FormatDateR(dateSearch1.Text);
                cmd.Parameters.Add("papp_date2", OracleDbType.Varchar2).Value = Utility.FormatDateR(dateSearch2.Text);
                cmd.Parameters.Add("papp_empno", OracleDbType.Varchar2).Value = "%" + empnoSearch.Text + "%";
                ...
                while (dr.Read())
                {
                    ...
                    row.Cells["papp_dut"].Value = dr["papp_dut"].ToString();
                    row.Cells["papp_pos"].Value = dr["papp_pos"].ToString();

                    row.Cells["papp_dept"].Value = dr["papp_dept"].ToString();
                    row.Cells["papp_sts"].Value = dr["papp_sts"].ToString();
                    ...
                    row.Cells["key4"].Value = Utility.GetCode(dr["papp_pap"].ToString());
        ...
        public void BtnInsert_Click()
        {
            ...
            //*--날짜 리셋----------------------------------------------
            //ct_papp_sdate.Text = System.DateTime.Now.ToString("yyyyMMdd");
            //*--사용여부 초기값-----------------------------------------
            //ct_papp_use.Text = "Y";
            //*--그룹코드 초기값-----------------------------------------
        }
        ...
        public void BtnDelete_Click()
        {
            ...
            DialogResult result = MessageBox.Show(row.Cells["papp_appno"].Value +
