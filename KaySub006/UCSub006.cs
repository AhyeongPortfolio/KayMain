    ...
    public partial class UserControl1: UserControl
    {
        ...
        public void BtnSave_Click()
        {
            ...
            try
            {
                ...
                foreach (DataGridViewRow row in dataGridView2.Rows)
                {
                    ...
                    if (row.Cells["status"].Value.Equals("U"))
                    {
                        ...
                        cmd.Parameters.Add("key1", OracleDbType.Varchar2).Value = row.Cells["key1"].Value;
                        cmd.Parameters.Add("key2", OracleDbType.Varchar2).Value = Utility.GetCode(row.Cells["key2"].Value.ToString());
                        cmd.Parameters.Add("key3", OracleDbType.Varchar2).Value = row.Cells["key3"].Value;
                    }
            ...
            foreach (DataGridViewRow row in dataGridView2.Rows)
            {
                ...
                row.Cells["key2"].Value = row.Cells["fam_rel"].Value;
                row.Cells["key3"].Value = row.Cells["fam_name"].Value;
        ...
        private void DataGridView1_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    row.Cells["Key2"].Value = dr["fam_rel"].ToString();
                    row.Cells["Key3"].Value = dr["fam_name"].ToString();
                    row.Cells["status"].Value = "";
                }
                dr.Close();
            }
        ...
        private void Input_Validation_Check(object sender, EventArgs e)
        {
            ...
            //*---------------------------------------------------------------------------------------------------------
            if (string.IsNullOrEmpty(ct_fam_empno.Text))
            {
                SetError(ct_fam_empno, "입력하실 사원을 선택해주세요", dataGridView2.SelectedRows[0], errorProvider1);
            }
            else
            {
                ResetError(ct_fam_empno, errorProvider1);
            }

            //*---------------------------------------------------------------------------------------------------------
            if (string.IsNullOrEmpty(ct_fam_name.Text))
            {
                SetError(ct_fam_name, "가족성함을 반드시 입력하세요", dataGridView2.SelectedRows[0], errorProvider1);
            }
            else
            {
