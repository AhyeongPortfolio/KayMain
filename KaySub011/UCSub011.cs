...
namespace KaySub011
{
    ...
    public partial class UserControl1: UserControl
    {
        ...
        public UserControl1()
        {
            ...
            ct_forl_empno.TextChanged += InputData_TextChanged;
            ct_forl_name.TextChanged += InputData_TextChanged;
            ct_forl_score.TextChanged += InputData_TextChanged;
            ct_forl_acqdate.TextChanged += InputData_TextChanged;
            ct_forl_organ.TextChanged += InputData_TextChanged;
            //*----Value Changed Event Handler(END)-----------------------------
            //*----Validated Event Handler(Start)-------------------------------
            ct_forl_name.Validated += Input_Validation_Check;
            ct_forl_empno.Validated += Input_Validation_Check;
            ct_forl_acqdate.Validated += Input_Validation_Check;
            //*----Validated Event Handler(END)---------------------------------
            //*----Enter Number Only(Start)-------------------------------------
            ct_forl_acqdate.KeyPress += Number_Only_Protect;
        ...
        private void UserControl1_Load(object sender, EventArgs e)
        {
            //*----ComboBox 채우기---------------------------------------------
            //*--콤보박스 채우기-----------------------------------------
            Utility.SetComboWithCdnm(ct_forl_name, SQLStatement.SelectSQL2);
        ...
        public void BtnInsert_Click()
        {
            ...
            if (dataGridView1.Rows.Count == 0) ct_forl_empno.ReadOnly = false;
            else ct_forl_empno.ReadOnly = true;

            ct_forl_empno.Text = dataGridView1.Rows[rowIdx2].Cells["bas_empno"].Value.ToString();
        ...
        public void BtnDelete_Click()
        {
            ...
            DialogResult result = MessageBox.Show(row.Cells["forl_name"].Value +
        ...
        public void BtnSave_Click()
        {
            ...
            try
            {
                ...
                foreach (DataGridViewRow row in dataGridView2.Rows)
                {
                    ...
                    cmd.Parameters.Add("forl_empno", OracleDbType.Varchar2).Value = row.Cells["forl_empno"].Value;
                    cmd.Parameters.Add("forl_name", OracleDbType.Varchar2).Value = row.Cells["forl_name"].Value.ToString();
                    cmd.Parameters.Add("forl_score", OracleDbType.Varchar2).Value = row.Cells["forl_score"].Value;
                    cmd.Parameters.Add("forl_acqdate", OracleDbType.Varchar2).Value = Utility.FormatDateR(row.Cells["forl_acqdate"].Value.ToString());
                    cmd.Parameters.Add("forl_organ", OracleDbType.Varchar2).Value = row.Cells["forl_organ"].Value;
            ...
            {
                if (row.Cells["status"].Value.Equals("")) continue;
                row.Cells["Key1"].Value = row.Cells["forl_empno"].Value;
                row.Cells["Key2"].Value = row.Cells["forl_name"].Value.ToString();
        ...
        private void DataGridView1_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    row.Cells["forl_empno"].Value = dr["forl_empno"].ToString();
                    row.Cells["forl_name"].Value = dr["forl_name"].ToString();
                    ...
                    row.Cells["Key1"].Value = dr["forl_empno"].ToString();
                    row.Cells["Key2"].Value = dr["forl_name"].ToString();
                    row.Cells["status"].Value = "";
                }
                dr.Close();
            }
        ...
        private void Input_Validation_Check(object sender, EventArgs e)
        {
            ...
            //*---------------------------------------------------------------------------------------------------------
            if (string.IsNullOrEmpty(ct_forl_empno.Text))
            {
                SetError(ct_forl_empno, "입력하실 사원을 선택해주세요", dataGridView2.SelectedRows[0], errorProvider1);
            }
            else
            {
                ResetError(ct_forl_empno, errorProvider1);
            }

            //*---------------------------------------------------------------------------------------------------------
            if (string.IsNullOrEmpty(ct_forl_acqdate.Text))
            {
                SetError(ct_forl_acqdate, "취득일자를 반드시 입력하세요", dataGridView2.SelectedRows[0], errorProvider1);
            }
            else
            {
                ResetError(ct_forl_acqdate, errorProvider1);
            }

            //*---------------------------------------------------------------------------------------------------------
            if (string.IsNullOrEmpty(ct_forl_name.Text))
            {
                SetError(ct_forl_name, "자격증명을 반드시 입력하세요", dataGridView2.SelectedRows[0], errorProvider1);
            }
            else
            {
                ResetError(ct_forl_name, errorProvider1);
            }
        }
